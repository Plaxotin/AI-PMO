...
def help_text(role: str = "user") -> str:
    """Краткая справка (v2.2.3).

    Блок «Вот что я умею» и кнопка «Закрыть поручение» удалены по требованию
    владельца: админам — только свободная форма, пользователям — одна строка
    про кнопку «Мои поручения».
    """
