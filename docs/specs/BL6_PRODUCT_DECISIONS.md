...
**Дата:** 2026-05-25
**Источник:** ответы Product на открытые вопросы Planner + спека v1.4
...
**Решение:** да — трек BL-6 описан только в `docs/specs/SPEC-BL-6-assignments-admin.md` (полный объём v1.4, фазы BL1-3 … BL1-5 обязательны). Аудит плана — отдельно в `docs/specs/SPEC-PLAN-AUDIT.md` (ранее общий файл `MVP_SPEC_AND_PLAN.md`).
...
## 8. Авторизация в MVP и post-MVP

**Решение (2026-05-25):** для **MVP** BL-6 обязательная пользовательская авторизация **не требуется**.

- BL1-1 реализует CRUD + журнал событий + optimistic locking без обязательной user session.
- `owner_id` в MVP допускается `null`; заполнение из auth-контекста переносится в post-MVP.
- Supabase Auth остаётся возможной опцией для последующих версий, но не является gate для текущих фаз MVP.
