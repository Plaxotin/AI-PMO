...
## 3.1 Загрузка видео/аудио на Vercel (лимит ~4,5 МБ)

На **Vercel Serverless** тело HTTP-запроса к `/ingest` ограничено платформой (**~4,5 МБ**), даже если в спеке BL2-0 указано до 500 МБ.

| Симптом | Причина |
|---------|---------|
| `Unexpected token 'R', "Request En"... is not valid JSON` | Ответ **413 Request Entity Too Large** (текст, не JSON) — файл слишком большой для Vercel |
| То же после исправления UI | Показано понятное сообщение о лимите ~4,5 МБ |

**Обход для пилота:** конвертировать запись в **mp3**, укоротить фрагмент или сжать видео **&lt; 4 МБ**. Текстовый smart-input и небольшие mp3 работают на production.

Полный лимит 500 МБ — при **self-host** (`npm run dev` / свой сервер без лимита Vercel).
